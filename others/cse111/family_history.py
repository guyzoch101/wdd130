...
def print_death_age(people_dict):
    ...
    for key in people_dict:
        inner_list = people_dict[key]
        name = inner_list[NAME_INDEX]
        death_age = inner_list[DEATH_YEAR_INDEX] - inner_list[BIRTH_YEAR_INDEX]
        
        print(f"{name} {death_age}")
    
    # for (key and value) in people_dict.items():
    # directly retrieves content in the dictionary
...
def count_genders(people_dict):
    ...
    # setting the initial value for male and female
    male = 0
    female = 0
    
    for key in people_dict:
        inner_list = people_dict[key]
        gender = inner_list[GENDER_INDEX]
...
def print_marriages(marriages_dict, people_dict):
    ...
    print("Marriages")
    for couple in marriages_dict:
        inner_list = marriages_dict[couple]
        marriage_year = inner_list[WEDDING_YEAR_INDEX]
        
        husband = inner_list[HUSBAND_KEY_INDEX]
        wife = inner_list[WIFE_KEY_INDEX]
        if (husband and wife) in people_dict:
            # husband
            husband_list = people_dict[husband]
            
            husband_name = husband_list[NAME_INDEX]
            husband_birthyear = husband_list[BIRTH_YEAR_INDEX]
            husband_marriage_age = marriage_year - husband_birthyear
            
            # wife
            wife_list = people_dict[wife]
            
            wife_name = wife_list[NAME_INDEX]
            wife_birthyear = wife_list[BIRTH_YEAR_INDEX]
            wife_marriage_age = marriage_year - wife_birthyear
            
            print(f"{husband_name} {husband_marriage_age} > {marriage_year} < {wife_name} {wife_marriage_age}")

    # add key not in dictionary part
    pass
